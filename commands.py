...
async def start(message: types.Message):
    ...
    if first_turn:
        await bot.send_message(message.from_user.id, f'Твой ход будет первым!')
        await await_player(player)
    else:
        await bot.send_message(message.from_user.id, f'Мой ход будет первым!')
        ...
        if 0 < int(message.text) < 29:
            ...
            if model.check_win(total):
                await bot.send_message(player.id, f'Победил {player.first_name}'
                                       f' Чтобы начать новую игру напиши /start, чтобы закончить игру напиши /stop')
                return
    ...
    if model.check_win(total):
        await bot.send_message(player.id, f'{player.first_name} ты проиграл,'
                                          f'тебя дёрнула железяка'
                                            f' Чтобы начать новую игру напиши /start, чтобы закончить игру напиши /stop')
        return
    if model.new_game_start(total):
        await bot.send_message(player.id, f'{player.first_name} Чтобы начать новую игру напиши /start')
        model.set_total_candies(150)
    model.set_total_candies(total)
